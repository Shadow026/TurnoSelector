...
const pool = new Pool({
    ...
    // Configuración para evitar conexiones muertas (BD remota)
    idleTimeoutMillis: 60000,       // Cerrar conexiones inactivas después de 60s
    connectionTimeoutMillis: 15000,  // Timeout más largo para conexiones remotas
    max: 10,                         // Máximo de conexiones simultáneas
    statement_timeout: 60000         // Timeout para queries
});

// Probar la conexión a la BD de forma asincrónica
(async () => {
    try {
...
app.get('/api/turnos', async (req, res) => {
    try {
        // Ordenar turnos en espera por prioridad (idservicio DESC) y luego por fecha (ASC)
        // idservicio=2 (compra productos) tiene prioridad sobre idservicio=1 (remesas/pagos)
        const result = await pool.query(
            'SELECT * FROM turnos ORDER BY idestado DESC, idservicio DESC, fechacreacion ASC'
        );
